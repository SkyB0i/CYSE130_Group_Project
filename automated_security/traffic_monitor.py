...
def main():
    ...
    if packet_counts > 1000:
        print("High traffic volume detected")

    for ip in source_ips:
        if source_ips[ip] > 1000 and ip not in white_list_IPS:
            print(f"High traffic volume detected from {ip}")
    for ip in destination_ips:
        if destination_ips[ip] > 1000 and ip not in white_list_IPS:
