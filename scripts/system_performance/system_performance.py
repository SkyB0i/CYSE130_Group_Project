...
log = 'logs/system_performance.log' #path for logging
...
def log_performance(cpu_use, memory_use, disk_use): #creates log entry of performance
    ...
    timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") # timestamp to the second for log
    log_entry = f"{timestamp} -- CPU: {cpu_use}%, Memory: {memory_use}%, Disk: {disk_use}%\n" # log entry format 
